...
import { CheckCircle, XCircle, Plus, Users, Wallet, Search, Calendar, MessageSquare, BarChart3, Send, TrendingUp, Download, Trash2, Lock, LogOut, IndianRupee, ArrowUpRight, EyeOff } from 'lucide-react';
  ...
  if (isExpired && userRole !== 'developer') {
    return (
      <div className="max-w-md mx-auto h-screen bg-slate-900 flex flex-col items-center justify-center p-10 text-center text-white">
        ...
        <h1 onClick={handleLogoClick} className="text-3xl font-black italic mb-2 cursor-pointer">GROWER</h1>
        <p className="text-slate-400 text-sm mb-8">License Expired.</p>
        <button onClick={() => window.open(`https://wa.me/${MY_PHONE}`, '_blank')} className="w-full bg-indigo-600 py-4 rounded-2xl font-bold uppercase text-[10px]">Renew Subscription</button>
      </div>
    );
  }
      ...
      {showPinModal && (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl flex items-center justify-center p-6 z-[100]">
          <div className="bg-white p-8 rounded-[3rem] w-full max-w-xs shadow-2xl text-center">
            ...
            <input type="password" placeholder="PIN" className="w-full bg-slate-100 p-4 rounded-2xl text-center text-lg font-bold outline-none mb-4 border-2 border-transparent focus:border-indigo-600" value={pinInput} onChange={(e) => setPinInput(e.target.value)} autoFocus />
            <button onClick={verifyPin} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-bold shadow-lg">Login</button>
          </div>
        </div>
      )}
        ...
        <header className="bg-indigo-600 text-white p-6 rounded-b-[3.5rem] shadow-xl">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-white/10 backdrop-blur-md rounded-[2rem] p-4 border border-white/20">
              <p className="text-[9px] font-black uppercase text-indigo-100 mb-1">Received</p>
              <p className="text-xl font-bold">₹{userRole === 'teacher' ? '---' : feesCollected}</p>
            </div>
            <div className="bg-white/10 backdrop-blur-md rounded-[2rem] p-4 border border-white/20">
              <p className="text-[9px] font-black uppercase text-indigo-100 mb-1">Due</p>
      ...
      <main className="p-4">
        {userRole === 'developer' && (
          <div className="space-y-6 text-center py-10">
            ...
            <h2 className="text-xl font-black">Developer Privacy</h2>
            ...
            {view === 'analytics' && userRole === 'owner' ? (
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-white p-5 rounded-[2rem] shadow-sm border border-slate-100">
                    ...
                    <p className="text-[9px] font-black text-slate-400 uppercase">Paid History</p>
                    <p className="text-2xl font-black text-slate-800">{getMonthlyHistory().length} Mo</p>
                  </div>
                </div>

                <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100">
                  <h3 className="text-[10px] font-black uppercase text-slate-400 mb-6 tracking-widest">Weekly Performance</h3>
                  <div className="flex items-end justify-between h-24 gap-2">
                    {attendanceTrend.map((day, i) => (
                      <div key={i} className="flex-1 flex flex-col items-center gap-2">
                        ...
                        <span className="text-[6px] font-bold text-slate-400">{day.date.split('-')[2]}/{day.date.split('-')[1]}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-indigo-50 p-6 rounded-[2.5rem] border border-indigo-100">
                  ...
                  {absentees.length > 0 ? absentees.map(s => (
                    <div key={s.id} className="bg-white p-3 rounded-2xl flex justify-between items-center shadow-sm mb-2">
                      ...
                      <button onClick={() => window.open(`https://wa.me/${s.phone}?text=Notice: ${s.name} was absent today.`, '_blank')} className="bg-emerald-500 text-white p-2 rounded-xl"><Send size={14}/></button>
                    </div>
                  )) : <p className="text-xs text-indigo-400 text-center py-2 italic">Full Attendance!</p>}
                </div>

                <button onClick={exportToCSV} className="w-full py-4 bg-slate-900 text-white rounded-[2rem] font-bold flex items-center justify-center gap-3"><Download size={20}/> Download CSV</button>
              </div>
              ...
              <div className="space-y-4">
                 <div className="bg-amber-50 p-6 rounded-[2.5rem] border border-amber-100 flex justify-between items-center">
                    <div>
                      <p className="text-[10px] font-black text-amber-800 uppercase mb-1">Pending Fees</p>
                      <p className="text-2xl font-black text-amber-600">₹{feesPending}</p>
                    </div>
                    <ArrowUpRight size={24} className="text-amber-700"/>
                 </div>
                 {filteredStudents.map(s => {
                   const isPaid = s.paidMonths.includes(currentMonthKey);
                   return (
                    <div key={s.id} className="bg-white p-5 rounded-[2rem] flex justify-between items-center shadow-sm border">
                      <div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-2">
                  <div className="flex-1 bg-white rounded-2xl shadow-sm flex items-center px-4 py-3 border">
                ...
                {showForm && (
                  <div className="p-6 bg-white rounded-[2.5rem] shadow-2xl border-t-4 border-indigo-600 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <input className="bg-slate-50 p-4 rounded-2xl text-sm font-bold outline-none border" placeholder="Phone" value={newPhone} onChange={e => setNewPhone(e.target.value)} />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <input className="bg-slate-50 p-4 rounded-2xl text-sm font-bold outline-none border" placeholder="Fee" type="number" value={newFee} onChange={e => setNewFee(e.target.value)} />
                    </div>
                    <button onClick={addStudent} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black uppercase text-xs tracking-widest shadow-lg">Register</button>
                  </div>
                )}
