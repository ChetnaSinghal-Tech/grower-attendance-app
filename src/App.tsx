...
interface Student {
  ...
  phone: string;
  paidMonths: string[];
  attendanceHistory: { [date: string]: 'present' | 'absent' | 'none' };
}

export default function App() {
  const [view, setView] = useState<
    'students' | 'finance' | 'analytics' | 'admin'
  >('students');
  ...
  const [expiryDate, setExpiryDate] = useState(() => {
    ...
    if (!saved) {
      ...
      trial.setDate(trial.getDate() + 7); // Default 7-day trial
    ...
    setStudents(
      students.map((s) => {
        if (s.id === id) {
          ...
          const newHist = {
            ...s.attendanceHistory,
            [selectedDate]: current === status ? 'none' : status,
          };
  ...
  const absentees = students.filter(
    (s) => s.attendanceHistory[selectedDate] === 'absent'
  );
  const monthlyRevenue = students
    .filter((s) => s.paidMonths.includes(currentMonthKey))
    .reduce((acc, s) => acc + s.feeAmount, 0);
  const totalLifetimeRevenue = students.reduce(
    (acc, s) => acc + s.paidMonths.length * s.feeAmount,
    0
  );
  ...
  const exportToCSV = () => {
    const headers =
      'Name,Phone,Class,MonthlyFee,Paid_Months,Total_Presents,Total_Absents\n';
    const rows = students
      .map((s) => {
        ...
        return `${s.name},${s.phone},${s.className},${
          s.feeAmount
        },"${s.paidMonths.join('; ')}",${presents},${absents}`;
      })
      .join('\n');
    ...
    return (
      <div className="max-w-md mx-auto h-screen bg-slate-900 flex flex-col items-center justify-center p-10 text-center text-white">
        ...
        <h1
          ...
          className="text-3xl font-black italic tracking-tighter mb-2 cursor-pointer select-none"
        >
          GROWER
        </h1>
        <p className="text-slate-400 text-sm mb-8">
          Subscription Expired. Please contact developer to renew.
        </p>
        <button
          onClick={() => window.open(`https://wa.me/${MY_PHONE}`, '_blank')}
          className="w-full bg-indigo-600 py-4 rounded-2xl font-bold"
        >
          Renew Access
        </button>
        {showPinModal && (
          <div className="fixed inset-0 bg-white text-slate-900 flex flex-col items-center justify-center p-6 z-[100]">
            ...
            <input
              type="password"
              placeholder="PIN"
              className="w-full bg-slate-100 p-4 rounded-xl text-center text-xl font-bold mb-4 outline-none border-2 border-indigo-600"
              value={pinInput}
  ...
  return (
    <div className="max-w-md mx-auto bg-slate-50 min-h-screen pb-32 font-sans text-slate-900 border-x border-slate-100">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 z-[100]">
          <div className="bg-white p-8 rounded-[2.5rem] w-full max-w-xs shadow-2xl">
            <input
              ...
              className="w-full bg-slate-100 p-4 rounded-xl text-center text-lg font-bold outline-none mb-4"
              value={pinInput}
              onChange={(e) => setPinInput(e.target.value)}
              autoFocus
            />
            ...
            <button
              ...
              className="w-full text-slate-400 mt-4 text-xs font-bold uppercase"
            >
              Cancel
            </button>
          </div>
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-2 bg-white/20 px-3 py-1.5 rounded-2xl border border-white/10">
            ...
            <input
              ...
              className="bg-transparent text-[10px] font-bold outline-none border-none text-white uppercase"
            />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20">
            <p className="text-[10px] font-bold uppercase text-indigo-100 mb-1">
        ...
        {view === 'admin' ? (
          <div className="space-y-6 animate-in zoom-in-95 duration-300">
            <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border-2 border-indigo-100">
              <h2 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                <Key size={20} /> Subscription Management
              </h2>
              ...
              <input
                ...
                className="w-full bg-slate-50 p-4 rounded-2xl font-bold outline-none mb-6"
              />
              <button
                onClick={() => setView('students')}
                className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black text-xs tracking-widest"
              >
                SAVE & EXIT
              </button>
            </div>
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <section>
              <h2 className="font-bold text-slate-700 mb-3 flex items-center gap-2 text-sm">
                <TrendingUp size={18} /> Monthly Collection
              </h2>
            ...
            <section className="bg-indigo-50 p-5 rounded-[2.5rem] border border-indigo-100 shadow-inner">
              <h2 className="font-bold text-indigo-900 mb-3 flex items-center gap-2 text-sm">
                <MessageSquare size={18} /> Bulk Nudges
              </h2>
              {absentees.length > 0 ? (
                  ...
                  {absentees.map((s) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() =>
                          window.open(
                            `https://wa.me/${s.phone}?text=Hello, ${s.name} was absent on ${selectedDate}.`,
                            '_blank'
                          )
                        }
                        className="bg-emerald-500 text-white p-2 rounded-xl active:scale-90 transition-transform"
                      >
                        <Send size={14} />
                      </button>
                    </div>
                ...
                <p className="text-[10px] text-indigo-400 italic text-center py-2">
                  No absentees to notify today.
                </p>
              )}
            </section>
            <button
              onClick={exportToCSV}
              className="w-full py-4 bg-slate-900 text-white rounded-3xl font-bold flex items-center justify-center gap-3 shadow-xl active:scale-95 transition-all"
            >
          ...
          <div className="space-y-4 animate-in fade-in duration-300">
            <h2 className="font-bold text-slate-700 px-2 flex justify-between items-center">
              ...
              <span className="text-[10px] bg-slate-200 px-3 py-1 rounded-full font-black uppercase tracking-tighter">
                {currentMonthKey}
              </span>
            </h2>
            {students.map((s) => {
              ...
              return (
                <div
                  key={s.id}
                  className="bg-white p-4 rounded-3xl flex justify-between items-center shadow-sm border border-slate-50 transition-all active:scale-[0.98]"
                >
                  <div>
                    <p className="font-bold text-sm text-slate-800">{s.name}</p>
                    <p className="text-[10px] text-slate-400 uppercase font-bold tracking-tight">
                      Class {s.className} • ₹{s.feeAmount}
                    </p>
                  </div>
                  <button
                    onClick={() => {
                      const newPaid = isPaid
                        ? s.paidMonths.filter((m) => m !== currentMonthKey)
                        : [...s.paidMonths, currentMonthKey];
                      setStudents(
                        students.map((item) =>
                          item.id === s.id
                            ? { ...item, paidMonths: newPaid }
                            : item
                        )
                      );
                    }}
                    className={`px-4 py-2 rounded-2xl text-[10px] font-black uppercase transition-all shadow-sm ${
                      isPaid
          ...
          <div className="space-y-4 animate-in fade-in duration-300">
            <div className="flex gap-2">
              ...
              <button
                ...
                className={`p-3 rounded-2xl shadow-lg active:scale-95 transition-all ${
                  showForm
                    ? 'bg-rose-500 text-white'
                    : 'bg-indigo-600 text-white'
                }`}
              >
                {showForm ? <Trash2 size={24} /> : <Plus size={24} />}
              </button>
            </div>
            ...
            {showForm && (
              <div className="p-5 bg-white rounded-[2.5rem] shadow-2xl border-t-4 border-indigo-600 animate-in slide-in-from-top-4 duration-300">
                ...
                <div className="grid grid-cols-2 gap-2 mb-2">
                  <input
                    className="bg-slate-50 p-3 rounded-2xl text-xs outline-none focus:ring-2 ring-indigo-100"
                ...
                <button
                  onClick={addStudent}
                  className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black uppercase text-xs tracking-widest shadow-lg shadow-indigo-200"
                >
                  Save & Register
                </button>
              </div>
            ...
            <div className="space-y-3">
              {students.map((s) => {
                ...
                return (
                  <div
                    key={s.id}
                    className={`p-5 rounded-[2.5rem] border-2 transition-all duration-300 ${
                      status === 'present'
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <div className="flex-1">
                        ...
                        <span className="text-[10px] bg-slate-100 text-slate-500 px-2.5 py-1 rounded-lg font-black uppercase tracking-tight">
                          Class {s.className}
                        </span>
                      </div>
                        ...
                        <button
                          ...
                          className={`p-2.5 rounded-2xl transition-all active:scale-90 ${
                            status === 'present'
                              ...
                              : 'text-slate-200 hover:text-slate-400 bg-slate-50'
                          }`}
                        >
                          <CheckCircle size={28} />
                        </button>
      ...
      <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[94%] max-w-md bg-white/90 backdrop-blur-xl p-2 rounded-full shadow-[0_20px_50px_rgba(0,0,0,0.1)] flex justify-between border border-white/50 z-50">
        <button
          onClick={() => setView('students')}
          className={`px-5 py-3 rounded-full font-black text-[10px] flex items-center gap-2 uppercase tracking-tighter transition-all ${
            view === 'students'
